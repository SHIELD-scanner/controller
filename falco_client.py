...
class FalcoAlertConsumer:
    def __init__(self, mongo_db, cluster_name: str, logger: logging.Logger, namespace: str = "falco-system"):
        self.db = mongo_db
    ...
    def sync_alert_to_mongo(self, alert: Dict[str, Any]):
        ...
        self.logger.info(f"Stored Falco alert: {alert.get('rule', 'Unknown')} - {alert.get('priority', 'Unknown')}")
    ...
    def watch_falco_logs(self):
        ...
        while self.running and retry_count < max_retries:
            try:
                ...
                if not pods.items:
                    self.logger.warning(f"No Falco pods found in namespace {self.namespace}")
                    time.sleep(30)
                    continue
                ...
                for event in w.stream(
                    ...
                    try:
                        ...
                        if not any(priority in log_line for priority in 
                ...
                retry_count += 1
                self.logger.warning(f"Error watching Falco logs (attempt {retry_count}/{max_retries}): {e}")
